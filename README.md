...
FastAPI is *async-native*, meaning it handles many simultaneous requests without blocking. This is critical because agent pipelines take 15–45 seconds — the server must stay responsive while they run in the background.
...
Incoming data is validated automatically by **Pydantic schemas** before it ever touches business logic. If a required field is missing or the wrong type, FastAPI rejects it with a clear error — no defensive code needed inside the agents.
...
**The solution — a restaurant analogy:**
...
LangGraph models the SOC workflow as a **state machine graph** — nodes are agents, edges are paths between them, and a shared state object carries data through the entire pipeline. Each agent reads from the state, does its work, and writes its output back.
...
|---|---|---|---|
| **Supervisor** | — | LangGraph FSM, no LLM needed | Routes between agents based on state |
...
| **Reporting** | Claude Haiku | Fast and cheap — just summarization | Executive summary + compiles full report |

**Why different models per agent?**
Cost and capability optimization. Claude Opus is the most capable but slowest and most expensive. We only spend Opus budget where deep reasoning is critical (triage, investigation). Haiku is fast and cheap — exactly right for summarization. This is a production pattern, not an accident.

**Structured tool use — why every agent uses it:**
...
This guarantees the output is always valid, typed, and parseable — no fragile regex or text extraction. If Claude tries to respond in prose, the API rejects it. This is what makes the pipeline reliable in production.
...
**How vectors work:** Every chunk of text is converted to a list of ~1024 numbers (a vector/embedding) that captures its meaning mathematically. Similar concepts produce similar numbers. When you search, your query also becomes a vector and Pinecone finds the closest matches. This is *semantic search* — it finds conceptually related content, not just keyword matches.

**What is indexed in Pinecone:**

| Source | What It Contains | Why It Matters |
|---|---|---|
| **MITRE ATT&CK** | Every known attack technique — descriptions, platforms, kill chain phases | Lets agents map alerts to TTPs with context |
...
- **Incident report view** — full structured report: triage, threat intel, investigation, remediation, executive summary
...
| **Embeddings** | Voyage-3 (Anthropic) | Text → vector conversion |
...
# Fetch MITRE ATT&CK, CISA KEV, and NVD CVEs → embed → upload to Pinecone
